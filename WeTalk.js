...
$httpClient.get({ url: buildUrl(path, acc.capture), headers: headers }, (error, response, data) => {
if (error) {
reject({ error: error });
} else {
resolve({ body: data });
...
const email = emailKeyOf(paramsRaw);
if (!email) {
const notifyTitle = '⚠️ 抓取失败';
const notifyBody = '请求里未取到 email 参数，无法识别账号。请确认已登录后再触发抓包。';
...
const notifyBody = '请先打开 WeTalk 触发抓包';
notify(notifyTitle, notifyBody);
console.log(`[通知] ${scriptName} | ${notifyTitle} | ${notifyBody}`);
$done();
} else {
