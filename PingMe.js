//2026/04/20
/*
@Name：PingMe 自动化签到+视频奖励+多账号
@Author：怎么肥事（Loon 适配版）
[MITM]
hostname = api.pingmeapp.net
[Script]
http-request ^https://api.pingmeapp.net/app/queryBalanceAndBonus script-path=https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/PingMe_Loon.js, require-body=false, timeout=10
cron "30 8,20 * * *" script-path=https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/PingMe_Loon.js, tag=PingMe签到, enabled=true
*/
...
setTimeout(() => {
i++;
fetchApi('videoBonus', acc).then(res => {
try {
...
}).then(() => fetchApi('queryBalanceAndBonus', acc)).then(res => {
try {
const d = JSON.parse(res.body);
...
const notifyTitle = existed ? '🔄 账号参数已更新' : '✅ 新账号已入库';
const notifyBody = `${alias}（id:${fp}）\n当前账号总数：${total}`;
notify(notifyTitle, notifyBody);
console.log(`[通知] ${scriptName} | ${notifyTitle} | ${notifyBody}`);
...
if (!ids.length) {
const notifyTitle = '⚠️ 未抓到任何账号';
const notifyBody = '请先打开 PingMe 触发抓包';
notify(notifyTitle, notifyBody);
console.log(`[通知] ${scriptName} | ${notifyTitle} | ${notifyBody}`);
$done();
} else {
...
const notifyBody = results.join('\n———\n');
notify(notifyTitle, notifyBody);
console.log(`[通知] ${scriptName} | ${notifyTitle} | ${notifyBody}`);
$done();
}).catch(err => {
const notifyTitle = '❌ 任务异常';
